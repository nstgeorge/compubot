...
from dis_snek import (InteractionContext, OptionTypes, Scale,
...
class Locate(Scale):
    ...
    @slash_command(name="locate", description="geolocate an IP address (THIS IS REAL, NOT A JOKE)")
    @slash_option(
        name="ip",
    ...
    async def locate(self, ctx: InteractionContext, ip: str):
        ...
        response = requests.post("https://api.m3o.com/v1/ip/Lookup", json=data, headers=headers)
        response_data = response.json()
        if response.ok:
            g_map = gen_google_maps_url(response_data['latitude'], response_data['longitude'])
